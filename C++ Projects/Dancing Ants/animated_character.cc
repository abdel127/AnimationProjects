                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
		S = Matrix4::Scale(Vector3(0.15, 0.2, 0.15));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    if (bone_name == "lfemur" || bone_name == "rfemur") {
		S = Matrix4::Scale(Vector3(0.08, 0.15, 0.08));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    if (bone_name == "ltibia" || bone_name == "rtibia") {
		S = Matrix4::Scale(Vector3(0.05, 0.35, 0.05));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    if (bone_name == "lfoot" || bone_name == "rfoot") {
		S = Matrix4::Scale(Vector3(0.08, 0.08, 0.08));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    ...
    if (bone_name == "thorax") {
		S = Matrix4::Scale(Vector3(0.13, 0.2, 0.13));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
    }
    if (bone_name == "head") {
		S = Matrix4::Scale(Vector3(0.1, 0.13, 0.1));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
		S = Matrix4::Translation(Vector3(0.06, 0.06, 0.08)) *
			Matrix4::Scale(Vector3(0.03, 0.04, 0.02));

		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(0, 0, 0));
    ...
    if (bone_name == "lclavicle" || bone_name == "rclavicle") {
		S = Matrix4::Scale(Vector3(0.17, 0.02, 0.02));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
	if (bone_name == "lhumerus" || bone_name == "rhumerus") {
		S = Matrix4::Scale(Vector3(0.05, 0.02, 0.02));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
	}
	**/
    ...
    if (bone_name == "lwrist" || bone_name == "rwrist") {
		S = Matrix4::Scale(Vector3(0.02, 0.02, 0.02));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
    }
    if (bone_name == "lhand" || bone_name == "rhand") {
		S = Matrix4::Scale(Vector3(0.08, 0.06, 0.02));
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, Color(1, 0.2, 0.2));
	}
