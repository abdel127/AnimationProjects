...
void QuakeApp::DrawUsingOpenGL() {
	...
	for (int x = start; x<end; x++) {
		...
		Color ecolor(e.magnitude() / 10.0, 0.388, 0.278);
		Vector3 epos(e.longitude() / 57.29578, e.latitude() / 57.29578, 0);

		Matrix4 Mquake =
			Matrix4::Translation(epos) *
			Matrix4::Scale(Vector3(e.magnitude()*3, e.magnitude()*3, e.magnitude())) *
			Matrix4::Scale(Vector3(0.001, 0.001, 0.001));

		quick_shapes_.DrawSphere(model_matrix * Mquake, view_matrix_, proj_matrix_, ecolor);
